...
export default function RightSide({ isMobile }) {
  ...
  const [selected, setSelected] = useState("Luau");
  const chips = ["Luau", "AI", "Websites", "Apps", "Other"];
  ...
  return (
    ...
    >
      ...
      <GlassCard>
        {notes
          .filter((note) => note.category === selected)
          .map((note, idx) => {
            ...
            useEffect(() => {
              ...
              const interval = setInterval(() => {
                current =
                  current.slice(0, i) + note.note[i] + current.slice(i + 1);
